...
import { Ionicons, AntDesign, Entypo, MaterialCommunityIcons, FontAwesome5 } from '@expo/vector-icons'; 
...
const Course = ({ course, index }) => {
          ...
          return (
                    <TouchableNativeFeedback background={TouchableNativeFeedback.Ripple('#c4c4c4')} useForeground onPress={() => navigation.navigate('NotificationsDetail', { course }) }>
                              <View style={styles.course}>
                                        <View style={styles.courseDesc}>
                                                  ...
                                                  <View style={{ flexDirection: 'row', alignItems: 'center'}}>
                                                            ...
                                                            <Text style={styles.courseTitle} numberOfLines={1}>{ course.NOM_CHAFFEUR } { course.PRENOM_CHAUFFEUR }</Text>
                                                  </View>
                                                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
...
export default function NotificationScreen() {
          ...
          const onRefresh = async () => {
                    setRefreshing(true)
                    const histories = await fetchApi(`/declarations/${user.DRIVER_ID}?corporate=${selectedCorporate.ID_CORPORATE}&month=${selectedMonth+1}&year=${selectedYear}&isAll=true`)
          ...
          useEffect(() => {
                    (async () => {
                              const url = `/declarations/${user.DRIVER_ID}?corporate=${selectedCorporate.ID_CORPORATE}&month=${selectedMonth+1}&year=${selectedYear}&isAll=true`
